...
app.post('/customers', (req, res) => {
    const { name, number,fees, gender, expiry_date, joining_date } = req.body;

    // insert new customer into customers table
    const sql = 'INSERT INTO customers(name, number, fees, gender, expiry_date, joining_date) VALUES (?, ?, ?, ?, ?, ?)';
    con.query(sql, [name, number, fees, gender, expiry_date, joining_date], (err, result) => {
      if (err) {
         console.log(err)
